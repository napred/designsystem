...
function getPlugins(entry, externals, updateBabelOptions, filename, bundleType) {
...
function handleRollupWarning(warning) {
  ...
  if (warning.code === 'UNUSED_EXTERNAL_IMPORT') {
    ...
    if (typeof importSideEffects[externalModule] !== 'boolean') {
      throw new Error(
        `An external module "${externalModule}" is used in a DEV-only code path ` +
...
async function build(bundle, bundleType) {
  ...
  const peerGlobals = Modules.getPeerGlobals(bundle.externals, bundle.moduleType);
  ...
  for (let i = 0; i < otherOutputPaths.length; i += 1) {
